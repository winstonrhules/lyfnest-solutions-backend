        ...
        try {
          ...
          if (admins.length > 0) {
            ...
            const adminParams = {
              ...
              Message: {
                Body: {
                  Html: {
                    ...
                    Data: `  
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                        <h2 style="color: #dc3545;">NEW INDEXED FORM SUBMISSION ALERT</h2> 
              ...
              Text: {
                Charset: "UTF-8",
                Data: `New Indexed Form submission from ${formData.firstName} ${formData.lastName} (${email})\n\nReview in admin panel.`
              }
            },
            Subject: {
              Charset: "UTF-8",
              Data: "🚨 New Indexed Form Submission Alert"
            }
          },
                   Source: process.env.SES_SENDER_EMAIL
                 };
    ...
    // Create notification
     try {
      ...
      await Notification.create({
        message: `New indexed form submission from ${formData.firstName} ${formData.lastName} at ${formattedSlot}`,
        formType: 'indexedForm',
        read: false
      });
    } catch (notifError) {
      console.error("Notification Creation Failed:", notifError);
        ...
        res.status(201).json({
      message: 'Indexed Universal Life Insurance Form Submission Successful', 
...
const deleteForm = asyncHandler(async (req, res) => {
  try {
    const form = await Iform.findById(req.params.id);
    if (!form) {
    ...
    await form.remove();
    res.status(200).json({ success: true, message: 'Indexed Form and appointment deleted successfully' });
  } catch (error) {
