...
// // Enhanced processUpcomingMeetings function with multiple matching strategies
// const processUpcomingMeetings = async (meetings, accessToken) => {
//   for (const meeting of meetings) {
//     try {
//       const meetingStartTime = new Date(meeting.start_time);
...
//       if (existingZoomMeeting) {
//         // Handle existing meeting updates (your existing logic)
//         continue;
//       }
...
//       // Get available appointments (contacted status, no existing zoom meeting)
//       const availableAppointments = await Appointment.find({
//         status: 'contacted',
//         'zoomMeeting.meetingId': { $exists: false },
//         lastContactDate: { $gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) }
...
//       // STRATEGY 1: Exact Meeting ID match (if you can embed appointment ID in meeting topic/description)
//       if (meeting.topic && meeting.topic.includes('[APPT:') && meeting.topic.includes(']')) {
//         const appointmentIdMatch = meeting.topic.match(/\[APPT:([a-f\d]{24})\]/);
//         if (appointmentIdMatch) {
//           const appointmentId = appointmentIdMatch[1];
//           matchedAppointment = availableAppointments.find(app => app._id.toString() === appointmentId);
//           if (matchedAppointment) {
//             matchReason = `embedded appointment ID: ${appointmentId}`;
//           }
//         }
//       }

//       // STRATEGY 2: Email domain matching (most reliable for unique emails)
//       if (!matchedAppointment && meeting.registrants && meeting.registrants.length > 0) {
//         const registrantEmail = meeting.registrants[0].email?.toLowerCase();
//         if (registrantEmail) {
//           matchedAppointment = availableAppointments.find(app => {
//             const userData = app.formData || app.formId || {};
//             const appointmentEmail = (userData.Email || userData.email || '').toLowerCase();
//             return appointmentEmail === registrantEmail;
//           });
//           if (matchedAppointment) {
//             matchReason = `exact email match: ${registrantEmail}`;
//           }
//         }
//       }

//       // STRATEGY 3: Phone number matching (if available in zoom registration)
//       if (!matchedAppointment && meeting.registrants && meeting.registrants.length > 0) {
//         const registrantPhone = meeting.registrants[0].phone;
//         if (registrantPhone) {
//           // Normalize phone numbers (remove all non-digits)
//           const normalizedRegPhone = registrantPhone.replace(/\D/g, '');
          
//           matchedAppointment = availableAppointments.find(app => {
//             const userData = app.formData || app.formId || {};
//             const appointmentPhone = (userData.phoneNumber || '').replace(/\D/g, '');
//             return appointmentPhone && appointmentPhone === normalizedRegPhone;
//           });
//           if (matchedAppointment) {
//             matchReason = `phone number match: ${normalizedRegPhone}`;
//           }
//         }
//       }

//       // STRATEGY 4: Time window + single appointment matching (most reliable fallback)
//       if (!matchedAppointment) {
//         // Find appointments within a reasonable time window (±2 hours)
//         const timeWindowStart = new Date(meetingStartTime.getTime() - (2 * 60 * 60 * 1000));
//         const timeWindowEnd = new Date(meetingStartTime.getTime() + (2 * 60 * 60 * 1000));
        
//         const timeMatchedAppointments = availableAppointments.filter(app => {
//           const appointmentTime = new Date(app.assignedSlot);
//           return appointmentTime >= timeWindowStart && appointmentTime <= timeWindowEnd;
//         });

//         if (timeMatchedAppointments.length === 1) {
//           matchedAppointment = timeMatchedAppointments[0];
//           matchReason = `unique appointment in 2-hour window around ${meetingStartTime.toISOString()}`;
//         }
//       }

//       // STRATEGY 5: Same day + single appointment matching
//       if (!matchedAppointment) {
//         const sameDayAppointments = availableAppointments.filter(app => {
//           const appointmentDate = new Date(app.assignedSlot);
//           const meetingDate = new Date(meetingStartTime);
          
//           return (
//             appointmentDate.getDate() === meetingDate.getDate() &&
//             appointmentDate.getMonth() === meetingDate.getMonth() &&
//             appointmentDate.getFullYear() === meetingDate.getFullYear()
//           );
//         });

//         if (sameDayAppointments.length === 1) {
//           matchedAppointment = sameDayAppointments[0];
//           matchReason = `single appointment on same day: ${meetingStartTime.toDateString()}`;
//         }
//       }

//       // STRATEGY 6: Fuzzy name matching (as last resort)
//       if (!matchedAppointment && meeting.topic) {
//         const topicLower = meeting.topic.toLowerCase();
...
//           // Must have meaningful names (at least 3 characters each)
//           if (firstName.length < 3 || lastName.length < 3) return false;
          
//           // Check if both names appear in topic (in any order)
//           return topicLower.includes(firstName) && topicLower.includes(lastName);
//         });
        
//         if (matchedAppointment) {
//           const userData = matchedAppointment.formData || matchedAppointment.formId || {};
//           matchReason = `fuzzy name match in topic: ${userData.firstName} ${userData.lastName}`;
//         }
//       }

//       // STRATEGY 7: Most recent contacted appointment (last resort)
//       if (!matchedAppointment && availableAppointments.length > 0) {
//         // Only if meeting is within last 24 hours and we have a recently contacted appointment
//         const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
//         const recentAppointments = availableAppointments.filter(app => 
//           new Date(app.lastContactDate) > oneDayAgo
//         );

//         if (recentAppointments.length === 1) {
//           matchedAppointment = recentAppointments[0];
//           matchReason = `most recent contacted appointment within 24 hours`;
//         }
//       }
...
//           createdAt: new Date(meeting.created_at) || new Date(),
//           syncedAt: new Date(),
//           matchingReason: matchReason // Store how it was matched for debugging
//         });
...
//           matchedAppointment._id,
//           {
//             // Only update time if significantly different (more than 30 minutes)
//             ...(Math.abs(new Date(matchedAppointment.assignedSlot).getTime() - meetingStartTime.getTime()) > 30 * 60 * 1000 ? {
//               assignedSlot: meetingStartTime,
//               contactWindowStart: meetingStartTime,
//               contactWindowEnd: meetingEndTime
...
//           const appointmentWithUser = await enrichAppointmentWithUser(updatedAppointment);
//           global.io.emit('updateAppointment', appointmentWithUser);
//           console.log(`📡 WebSocket update emitted for booking: ${matchedAppointment._id} (${matchReason})`);
//         }

//         // Log successful match for monitoring
//         console.log(`🎯 BOOKING MATCHED: ${matchReason}`);
//         console.log(`   Appointment: ${matchedAppointment._id} (${matchedAppointment.user?.firstName} ${matchedAppointment.user?.lastName})`);
//         console.log(`   Meeting: ${meeting.id} ("${meeting.topic}")`);
//         console.log(`   Time: ${meetingStartTime.toISOString()}`);
//       } else {
//         console.log(`❌ No match found for meeting ${meeting.id} - Topic: "${meeting.topic}"`);
//         console.log(`   Available appointments: ${availableAppointments.length}`);
//         console.log(`   Meeting time: ${meetingStartTime.toISOString()}`);
//       }
...
// // // Enhanced scheduler URL generation with embedded appointment ID
// // const generateSchedulerUrl = (meeting, firstName, lastName, email, appointmentId) => {
// //   const encodedFirstName = encodeURIComponent(firstName);  
// //   const encodedLastName = encodeURIComponent(lastName); 
// //   const encodedEmail = encodeURIComponent(email);

// //   // Embed appointment ID in the URL for reliable matching later
// //   return `https://scheduler.zoom.us/nattye-a/discovery-and-guidance-call?meeting_id=${meeting.id}&first_name=${encodedFirstName}&last_name=${encodedLastName}&email=${encodedEmail}&appt_id=${appointmentId}`;
// // };

// // // Update your safeUniversalContactUserByEmail function to use the enhanced URL
// // const updateMeetingCreationInContactFunction = (meetingData, userName, userEmail, appointmentId) => {
// //   // In your existing safeUniversalContactUserByEmail function, 
// //   // replace the schedulerUrl generation with:
  
// //   const schedulerUrl = generateSchedulerUrl(
// //     meeting, 
// //     firstName, 
// //     lastName, 
// //     userEmail, 
...
// //   // Also update the meeting topic to include appointment ID
// //   const enhancedMeetingData = {
// //     ...meetingData,
// //     topic: `${getFormTypeName(appointment.formType)} Consultation - ${userName} [APPT:${appointmentId}]`
// //   };

// //   return { schedulerUrl, enhancedMeetingData };
// // };

// // // Monitoring function to check matching effectiveness
// // const getMatchingStats = async () => {
// //   const zoomMeetings = await ZoomMeeting.find({ matchingReason: { $exists: true } });
// //   const stats = {};
  
// //   zoomMeetings.forEach(meeting => {
// //     const reason = meeting.matchingReason || 'unknown';
// //     stats[reason] = (stats[reason] || 0) + 1;
// //   });
  
// //   console.log('🔍 Matching Strategy Effectiveness:');
// //   Object.entries(stats).forEach(([strategy, count]) => {
...
// ====== ENHANCED ZOOM SERVICE - NO WEBHOOKS ======
const axios = require('axios');
...
const safeUniversalContactUserByEmail = async (req, res) => {
  try {
    ...
    // ENHANCED: Generate a more robust token with appointment ID embedded
    const appointmentToken = crypto.createHash('sha256')
      .update(`${appointmentId}-${Date.now()}-${crypto.randomBytes(8).toString('hex')}`)
      .digest('hex').substring(0, 32);

    console.log(`Generated token for appointment ${appointmentId}: ${appointmentToken}`);
    ...
    try {
      ...
      // ENHANCED: Create meeting with embedded appointment ID and token in topic for reliable matching
      const meetingData = {
        topic: `${getFormTypeName(appointment.formType)} Consultation - ${userName} [APPT:${appointmentId}][TOKEN:${appointmentToken}]`,
        type: 2, // Scheduled meeting
        start_time: new Date(appointment.assignedSlot).toISOString(),
        duration: 60, // 60 minutes
        timezone: 'America/New_York',
        agenda: `Appointment ID: ${appointmentId} | Token: ${appointmentToken} | Original Email: ${userEmail}`, // Additional fallback
        settings: {
          host_video: true,
          participant_video: true,
          join_before_host: false,
      ...
      // ENHANCED: Create scheduler URL with token for reliable matching
      const encodedFirstName = encodeURIComponent(firstName);  
      const encodedLastName = encodeURIComponent(lastName); 
      const encodedEmail = encodeURIComponent(userEmail);
      const encodedToken = encodeURIComponent(appointmentToken);
      ...
      // Include multiple identifiers in the scheduler URL
      const schedulerUrl = `https://scheduler.zoom.us/nattye-a/discovery-and-guidance-call?meeting_id=${meeting.id}&first_name=${encodedFirstName}&last_name=${encodedLastName}&email=${encodedEmail}&token=${encodedToken}&appointment_id=${encodedAppointmentId}&original_email=${encodedEmail}`;

      // Save meeting to ZoomMeeting model with enhanced token data
      const zoomMeeting = new ZoomMeeting({
        appointment: appointmentId,
        meetingId: meeting.id,
        joinUrl: meeting.join_url,
        ...
        schedulerUrl: schedulerUrl,
        appointmentToken: appointmentToken, // Store the token
        originalEmail: userEmail, // Store original email for matching
        originalName: userName, // Store original name for reference
        topic: meeting.topic,
      ...
      schedulerLink = zoomMeeting.schedulerUrl;
      console.log('Zoom meeting created with embedded identifiers:', meeting.id);
    } catch (zoomError) {
      console.error('Failed to create Zoom meeting:', zoomError.response?.data || zoomError.message);
    }

    // Get form data safely (keeping your existing logic)
    const getFormDataSafely = () => {
      try {
        if (appointment.formType === 'termForm' && appointment.formId) {
          return appointment.formId;
    ...
    // Generate email content (keeping your existing logic)
    const generateSafeContent = (formType, formData) => {
      const baseGreeting = `Hi ${userName},\n\nThank you for submitting your request! I'm following up to schedule your consultation.`;
    ...
    // Generate HTML email (keeping your existing HTML generation logic)
    const generateSafeHTMLContent = (formType, formData, emailMessage, schedulerLink, zoomMeetingData) => {
      const generateSafeFormDetailsHTML = (formType, formData) => {
        if (!formData) return '';
            ...
            <div style="padding: 30px; font-size: 16px; line-height: 1.6; color: #2f4f4f;">
                      ...
                      <div style="background: #e3f2fd; padding: 20px; margin: 20px 0; border-radius: 8px; border-left: 4px solid #2196f3;">
                        ...
                        <ul style="color: #555; margin: 10px 0;">
                          <li><strong>Meeting ID:</strong> ${zoomMeetingData.meetingId}</li>
                          <li><strong>Scheduled Time:</strong> ${new Date(appointment.assignedSlot).toLocaleString()}</li>
                          <li><strong>Reference Token:</strong> ${appointmentToken.substring(0, 8)}...</li>
                        </ul>
                      </div>
                      ` : ''}
    ...
    // Send email using SES (keeping your existing SES setup)
    const { SESClient, SendEmailCommand } = require("@aws-sdk/client-ses");
    ...
    const params = {
      ...
      Message: {
        Body: {
          ...
          Text: {
            Charset: "UTF-8",
            Data: `${emailMessage}\n\n${zoomMeetingData ? 'Confirm meeting' : 'Schedule your meeting'}: ${schedulerLink}\n\nReference Token: ${appointmentToken}`
          }
        },
        Subject: {
          Charset: "UTF-8",
    ...
    // ENHANCED: Update appointment with token and original data
    const updateData = {
      status: 'contacted',
      lastContactDate: new Date(),
      contactMethod: contactMethod,
      contactedBy: adminName || 'Admin',
      appointmentToken: appointmentToken, // Critical: Store token in appointment
      originalEmail: userEmail, // Store for matching
      originalName: userName, // Store for reference
      schedulerLinkSentCount: (appointment.schedulerLinkSentCount || 0) + 1,
      schedulerLinkLastSent: new Date()
    };
    ...
    console.log(`Appointment ${appointmentId} updated with token: ${appointmentToken}`);
    ...
    res.status(200).json({
      ...
      zoomMeeting: zoomMeetingData,
      appointmentId,
      appointmentToken, // Return token for debugging
      contactMethod: 'email',
      sentAt: new Date(),
      recipient: userEmail,
      schedulerLink: schedulerLink,
    ...
    if (!appointment) {
      ...
      return res.status(200).json({
        ...
        appointment: {
          _id: appointment._id,
          user: {
            firstName: appointment.originalName?.split(' ')[0] || appointment.user?.firstName || appointment.formData?.firstName || appointment.formId?.firstName || '',
            lastName: appointment.originalName?.split(' ').slice(1).join(' ') || appointment.user?.lastName || appointment.formData?.lastName || appointment.formId?.lastName || '',
            email: appointment.originalEmail || appointment.user?.email || appointment.formData?.Email || appointment.formData?.email || appointment.formId?.Email || appointment.formId?.email || ''
          },
          formType: appointment.formType,
          assignedSlot: appointment.assignedSlot,
...
// ENHANCED: Priority-based appointment matching with token support (NO WEBHOOKS)
const processUpcomingMeetings = async (meetings, accessToken) => {
  console.log(`Processing ${meetings.length} upcoming meetings for appointment matching...`);
  ...
  for (const meeting of meetings) {
    try {
        ...
        if (appointment && appointment.assignedSlot.getTime() !== meetingStartTime.getTime()) {
          ...
          const updatedAppointment = await Appointment.findByIdAndUpdate(
            appointment._id,
            {
              ...
              contactWindowStart: meetingStartTime,
              contactWindowEnd: meetingEndTime,
              status: 'booked', // Meeting exists, so it's booked
              customerBookedAt: new Date(),
              lastUpdated: new Date()
            },
            { runValidators: true, new: true }
          ...
          if (global.io && updatedAppointment) {
            const appointmentWithUser = await enrichAppointmentWithUser(updatedAppointment);
            global.io.emit('updateAppointment', appointmentWithUser);
            console.log('Updated appointment status via sync:', appointmentWithUser._id);
          }
        }
        continue;
      }
      ...
      // PRIORITY 1: Token-based matching (most reliable)
      if (meeting.topic && meeting.topic.includes('[TOKEN:') && meeting.topic.includes(']')) {
        const tokenMatch = meeting.topic.match(/\[TOKEN:([a-f\d]{32})\]/);
        if (tokenMatch) {
          const token = tokenMatch[1];
          console.log(`Found token in meeting topic: ${token}`);
          
          matchedAppointment = availableAppointments.find(app => app.appointmentToken === token);
          if (matchedAppointment) {
            matchReason = `token match: ${token}`;
            console.log(`PRIORITY 1 MATCH: Token-based matching successful`);
          }
        }
      }

      // PRIORITY 2: Embedded appointment ID matching (very reliable)
      if (!matchedAppointment && meeting.topic && meeting.topic.includes('[APPT:') && meeting.topic.includes(']')) {
        const appointmentIdMatch = meeting.topic.match(/\[APPT:([a-f\d]{24})\]/);
        if (appointmentIdMatch) {
          const appointmentId = appointmentIdMatch[1];
          matchedAppointment = availableAppointments.find(app => app._id.toString() === appointmentId);
          if (matchedAppointment) {
            matchReason = `embedded appointment ID: ${appointmentId}`;
            console.log(`PRIORITY 2 MATCH: Appointment ID matching successful`);
          }
        }
      }

      // PRIORITY 3: Original email matching (reliable for unique emails)
      if (!matchedAppointment) {
        // Try to get registrant email from Zoom API
        try {
          ...
          if (registrants.length > 0) {
            const registrantEmail = registrants[0].email?.toLowerCase();
            if (registrantEmail) {
              // First try matching with original email stored in appointment
              matchedAppointment = availableAppointments.find(app => 
                app.originalEmail && app.originalEmail.toLowerCase() === registrantEmail
              );
              
              if (matchedAppointment) {
                matchReason = `original email match: ${registrantEmail}`;
                console.log(`PRIORITY 3 MATCH: Original email matching successful`);
              } else {
                // Fallback to form data email matching
                matchedAppointment = availableAppointments.find(app => {
                  const userData = app.formData || app.formId || {};
                  const appointmentEmail = (userData.Email || userData.email || '').toLowerCase();
                  return appointmentEmail === registrantEmail;
                ...
                if (matchedAppointment) {
                  matchReason = `form data email match: ${registrantEmail}`;
                  console.log(`PRIORITY 3 MATCH: Form data email matching successful`);
                }
              }
            }
          }
        } catch (regError) {
          console.log(`Could not fetch registrants for meeting ${meeting.id}:`, regError.message);
        }
      }

      // PRIORITY 4: Time window + single appointment matching
      if (!matchedAppointment) {
        const timeWindowStart = new Date(meetingStartTime.getTime() - (2 * 60 * 60 * 1000));
        const timeWindowEnd = new Date(meetingStartTime.getTime() + (2 * 60 * 60 * 1000));
        ...
        if (timeMatchedAppointments.length === 1) {
          matchedAppointment = timeMatchedAppointments[0];
          matchReason = `unique appointment in 2-hour window`;
          console.log(`PRIORITY 4 MATCH: Time-based matching successful`);
        }
      }

      // PRIORITY 5: Same day + single appointment matching
      if (!matchedAppointment) {
        const sameDayAppointments = availableAppointments.filter(app => {
          const appointmentDate = new Date(app.assignedSlot);
          const meetingDate = new Date(meetingStartTime);
        ...
        if (sameDayAppointments.length === 1) {
          matchedAppointment = sameDayAppointments[0];
          matchReason = `single appointment on same day: ${meetingStartTime.toDateString()}`;
          console.log(`PRIORITY 5 MATCH: Same day matching successful`);
        }
      }

      // PRIORITY 6: Most recent contacted appointment (last resort)
      if (!matchedAppointment && availableAppointments.length > 0) {
        const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const recentAppointments = availableAppointments.filter(app => 
          new Date(app.lastContactDate) > oneDayAgo
        ...
        if (recentAppointments.length === 1) {
          matchedAppointment = recentAppointments[0];
          matchReason = `most recent contacted appointment within 24 hours`;
          console.log(`PRIORITY 6 MATCH: Recent appointment matching successful`);
        }
      }

      if (matchedAppointment) {
        console.log(`APPOINTMENT MATCHED via ${matchReason}:`);
        console.log(`   Appointment: ${matchedAppointment._id}`);
        ...
        const newZoomMeeting = new ZoomMeeting({
          ...
          createdAt: new Date(meeting.created_at) || new Date(),
          syncedAt: new Date(),
          matchingReason: matchReason,
          appointmentToken: matchedAppointment.appointmentToken, // Preserve token
          originalEmail: matchedAppointment.originalEmail,
          originalName: matchedAppointment.originalName,
          topic: meeting.topic,
          agenda: meeting.agenda,
          duration: meeting.duration
        });
        ...
        const updatedAppointment = await Appointment.findByIdAndUpdate(
          matchedAppointment._id,
          {
            ...
            status: 'booked',
            customerBookedAt: new Date(),
            clientViewedScheduler: true,
            clientViewedSchedulerAt: new Date(),
            zoomMeeting: {
              id: meeting.id,
              meetingId: meeting.id,
              topic: meeting.topic,
        ...
        if (global.io && updatedAppointment) {
          const appointmentWithUser = await enrichAppointmentWithUser(updatedAppointment);
          global.io.emit('updateAppointment', appointmentWithUser);
          console.log(`WebSocket update emitted for booking: ${matchedAppointment._id}`);
        }

        // Create notification
        await Notification.create({
          message: `Client booked meeting via ${matchReason}: ${matchedAppointment.originalName || 'Client'} - ${meetingStartTime.toLocaleDateString()} at ${meetingStartTime.toLocaleTimeString()}`,
          formType: matchedAppointment.formType || 'meeting_scheduled',
          read: false,
          appointmentId: matchedAppointment._id
        });

        console.log(`Successfully processed booking with ${matchReason}`);
      } else {
        console.log(`No match found for meeting ${meeting.id}`);
        console.log(`   Topic: "${meeting.topic}"`);
        console.log(`   Available appointments: ${availableAppointments.length}`);
        console.log(`   Meeting time: ${meetingStartTime.toISOString()}`);
        
        // Log available appointment tokens for debugging
        if (availableAppointments.length > 0) {
          console.log(`   Available appointment tokens:`);
          availableAppointments.forEach(app => {
            console.log(`     - ${app._id}: token=${app.appointmentToken || 'NO_TOKEN'}, email=${app.originalEmail || 'NO_EMAIL'}`);
          });
        }
      }
...
// ENHANCED: Meeting completion detection (keeping your existing logic)
const processPastMeetings = async (pastMeetings, accessToken) => {
  console.log('Processing past meetings for completion detection...');
  ...
  for (const meeting of pastMeetings) {
    try {
      ...
      if (participants.length === 0) {
        ...
        if (singleParticipant.duration >= 5) {
          wasCompleted = true;
          console.log(`Meeting ${meeting.id} marked as COMPLETED - Single participant stayed ${singleParticipant.duration} minutes`);
        } else {
          wasCompleted = false;
          console.log(`Meeting ${meeting.id} marked as MISSED - Single participant only stayed ${singleParticipant.duration} minutes`);
        }
        ...
        if (meaningful.length < 2) {
          wasCompleted = false;
          console.log(`Meeting ${meeting.id} marked as MISSED - Not enough meaningful participants`);
        } else {
          const hasRealEngagement = meaningful.some(p => p.duration >= 2);
          
          if (!hasRealEngagement) {
            wasCompleted = false;
            console.log(`Meeting ${meeting.id} marked as MISSED - No real engagement`);
          } else {
            const totalEngagementTime = meaningful.reduce((sum, p) => sum + p.duration, 0);
      ...
      // Create notification
      const userName = appointment.originalName || 
...
// Enhanced syncZoomMeetings with completion detection
const syncZoomMeetingsWithCompletion = async () => {
  try {
    console.log('Starting enhanced Zoom meeting sync with completion detection...');
    const accessToken = await getZoomAccessToken();
...
const enrichAppointmentWithUser = async (appointment) => {
  ...
  // Try original data first (most reliable)
  if (appointment.originalName) {
    const nameParts = appointment.originalName.split(' ');
    userData.firstName = nameParts[0] || userData.firstName;
    userData.lastName = nameParts.slice(1).join(' ') || userData.lastName;
  }
  if (appointment.originalEmail) {
    userData.email = appointment.originalEmail;
  }
...
module.exports = {      
  ...
  manualSync: async (req, res) => { 
    try {
      await syncZoomMeetingsWithCompletion();
      res.status(200).json({ message: 'Enhanced manual sync completed' });
    } catch (error) {  
      console.error('Enhanced manual sync error:', error);
      res.status(500).json({ error: 'Manual sync failed', details: error.message });
