...
/**
 * POST /api/scheduled-emails/schedule
...
router.get('/', async (req, res) => {
  try {
    ...
    const scheduledEmails = await emailScheduler.getAllScheduledEmails(filters);
...
/**
 * GET /api/scheduled-emails/:jobId
...
/**
 * PUT /api/scheduled-emails/:jobId
...
router.put('/:jobId', async (req, res) => {
  try {
    ...
    const updatedSchedule = await emailScheduler.updateScheduledEmail(jobId, updates);
...
/**
 * POST /api/scheduled-emails/:jobId/send-now
...
router.post('/:jobId/send-now', async (req, res) => {
  try {
    ...
    const schedule = await emailScheduler.sendImmediately(jobId);
    ...
    if (error.message.includes('already been sent') || error.message.includes('currently being processed')) {
      return res.status(400).json({
        success: false,
        error: error.message
      });
...
/**
 * DELETE /api/scheduled-emails/:jobId
...
router.delete('/:jobId', async (req, res) => {
  try {
    ...
    const schedule = await emailScheduler.cancelScheduledEmail(jobId);
...
/**
 * POST /api/scheduled-emails/scheduler/trigger
