...
class EmailSchedulerService {
  ...
  async cleanupStaleLocks() {
    try {
      // FIXED: Don't check dbConnected here, mongoose will handle it
      const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);
   ...
   */
  async scheduleEmail(emailData) {
    // FIXED: Removed database connection check - let mongoose handle it
    const jobId = uuidv4();
  ...
  /**
   * Get all scheduled emails - FIXED VERSION
   */
  async getAllScheduledEmails() {
    try {
      // FIXED: Removed strict database check, let mongoose handle connection
      const emails = await EmailSchedule.find()
        .sort({ scheduledFor: -1 }) // Sort by most recent first
        .limit(100);
