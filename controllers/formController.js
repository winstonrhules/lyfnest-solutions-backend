...
const sendEmailVerification = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ 
      ...
      expiresAt,
      emailVerificationId: emailVerification._id  // Return ID for reference
    });
  } catch (error) {
    console.error('SES Error:', error);
    res.status(500).json({ error: 'Failed to send verification email' });
...
const verifyCode = asyncHandler(async (req, res) => {
  try {
    ...
    if (!verification) {
      return res.status(400).json({ error: 'Verification not found or expired' });
    }
...
const verifyEmailCode = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({ 
    ...
    emailVerificationId: record._id  // Return ID for form submission
  });
});
...
const submissionForm = asyncHandler(async (req, res) => {
  try {
    const { 
      verification: phoneVerificationId, 
    ...
    // Phone verification check
    const phoneVerification = await Verification.findById(phoneVerificationId);
    const phoneWindow = new Date(Date.now() - VERIFICATION_WINDOW_MINUTES * 60 * 1000);

    if (!phoneVerification || phoneVerification.status !== 'verified' || phoneVerification.verifiedAt < phoneWindow) {
      return res.status(400).json({ error: 'Phone verification required or expired' });
    }

    // Email verification check
    const emailVerification = await EmailVerification.findById(emailVerificationId);
    const emailWindow = new Date(Date.now() - VERIFICATION_WINDOW_MINUTES * 60 * 1000);

    if (!emailVerification || emailVerification.status !== 'verified' || emailVerification.verifiedAt < emailWindow) {
      return res.status(400).json({ error: 'Email verification required or expired' });
    }
    ...
    const slotTime = appointmentDetails.assignedSlot 
      ? appointmentDetails.assignedSlot.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      : 'soon';
      
    const notificationMessage = `New submission from ${newForm.firstName} ${newForm.lastName} at ${slotTime}`;

    // Send user confirmation email
    let userEmailSent = false;
    try {
      const userParams = {
        Destination: { ToAddresses: [newForm.Email] },
        Message: {
          Body: {
            Html: {
              Charset: "UTF-8",
      ...
      console.error("User Email Failed:", {
        error: userMailError,
        recipient: newForm.Email,
        message: "User confirmation email failed"
      });
    }
    ...
    let adminAlertSent = false;
    try {
      const admins = await User.find({ role: "admin", emailVerified: true }).select("email");
      const adminEmails = admins.map(admin => admin.email);
      
      if (adminEmails.length > 0) {
        const formattedSlot = appointmentDetails.assignedSlot 
          ? appointmentDetails.assignedSlot.toLocaleString('en-US', {
              weekday: 'short',
              month: 'short',
              day: 'numeric',
              hour: '2-digit',
      ...
      console.error('Admin Email Failed:', {
        ...
        message: "Admin alert email failed"
      });
    }
    ...
    res.status(201).json({
      ...
      appointment: {
        slot: appointmentDetails.assignedSlot,
        window: `${appointmentDetails.contactWindowStart.toISOString()} - ${appointmentDetails.contactWindowEnd.toISOString()}`
      }
    });
