...
const ZoomMeeting = require('../models/zoomMeetingModels');
const Appointment = require('../models/appointmentModels'); // Add this import
const Notification = require('../models/notificationModels'); // Add this import
...
const createZoomMeeting = async (req, res) => {
  try {
    ...
    const firstName = appointment.user?.firstName || appointment.formData?.firstName || '';
    const lastName = appointment.user?.lastName || appointment.formData?.lastName || '';
    const email = appointment.user?.email || appointment.formData?.Email || appointment.formData?.email || '';
    ...
    appointment.zoomMeeting = zoomMeeting._id;
    appointment.appointmentToken = appointmentToken; // Store token in appointment too
    await appointment.save();
    ...
    res.status(201).json({
      ...
      meetingInfo: {
        ...
        password: meeting.password,
        schedulerUrl: zoomMeeting.schedulerUrl,
        appointmentToken: appointmentToken // Return token for testing
      }
    });
