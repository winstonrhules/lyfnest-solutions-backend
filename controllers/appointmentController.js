...
//           <h1 style="font-family: 'Poppins', sans-serif; font-size: 28px; font-weight: 600; text-align: center; margin: 0; color: #0e94d0; letter-spacing: 1.5px; position: relative; z-index: 2;">📅 SCHEDULE A MEETING </h1>
//         </div>
...
//     const schedulingLink = `${process.env.CONTACT_URL}/book-contact-meeting/${appointment._id}`;
...
const scheduleContactListZoomMeeting = asyncHandler(async (req, res) => {
  try {
    // const { contactId, contactData } = req.body;
    const { contactId, contactData, emailSubject, emailMainMessage, emailClosingMessage } = req.body;
    ...
    // Generate HTML email with the two separately editable sections
    const htmlContent = generateContactListEmail(contact, schedulingLink, initialSlot, emailMainMessage, emailClosingMessage);
    const textContent = `Hi ${contact.firstName},\n\n${emailMainMessage || `It's time for your annual ${contact.policyType} policy review.`}\n\n${schedulingLink}\n\n${emailClosingMessage || `If you have any questions before scheduling, please don't hesitate to reach out.`}\n\nBest regards,\nYour Insurance Team`;
...
function generateContactListEmail(contact, schedulingLink, suggestedDate, emailMainMessage = null, emailClosingMessage = null) {
  const formattedDate = new Date(suggestedDate).toLocaleDateString('en-US', {
    weekday: 'long', year: 'numeric', month: 'long', day: 'numeric'
  });

  // Use custom or default for each editable section independently
  const mainMessageHtml = emailMainMessage
    ? `<p>${emailMainMessage.replace(/\n/g, '<br>')}</p>`
    : `<p>It's time for your annual ${contact.policyType} policy review. We want to ensure your coverage continues to meet your needs and discuss any changes that may benefit you.</p>`;

  const closingMessageHtml = emailClosingMessage
    ? `<p>${emailClosingMessage.replace(/\n/g, '<br>')}</p>`
    : `<p>If you have any questions before scheduling, please don't hesitate to reach out.</p>`;
