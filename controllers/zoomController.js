...
const createZoomMeeting = async (req, res) => {
  try {
    const { appointmentId, startTime } = req.body;
      const clientTimezone = req.body.timezone || 'America/New_York';
...
const parseZoomDate = (dateString, timezone = 'UTC') => {
  if (!dateString) return null;
  
  try {
    // Parse the date string with explicit timezone handling
    let date = new Date(dateString);
    
    // If the date string doesn't include timezone info, assume it's in the provided timezone
    if (dateString.indexOf('Z') === -1 && dateString.indexOf('+') === -1) {
      // Convert to ISO string with timezone offset
      const isoString = new Date(dateString + 'Z').toISOString();
    ...
    console.log(`Found ${meetings.length} Zoom meetings`);
      ...
      //   const meetingEndTime = new Date(meetingStartTime.getTime() + (meetingDuration * 60000));
       for (const meeting of meetings) {
            try {
        // Get the meeting timezone, default to America/New_York if not provided
        const meetingTimezone = meeting.timezone || 'America/New_York';
        
        // Parse the date with timezone consideration
        const meetingStartTime = parseZoomDate(meeting.start_time, meetingTimezone);
        
        if (!meetingStartTime) continue;

        // Convert to your server's timezone for consistent storage
        const meetingDuration = meeting.duration || 60;
        const meetingEndTime = new Date(meetingStartTime.getTime() + (meetingDuration * 60000));
