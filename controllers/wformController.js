...
const submissionForm = asyncHandler(async (req, res) => {
  ...
  const savedForm = await newWform.save();
   ...
   let userEmailSent = false;
      try {
       ...
       let adminAlertSent = false;
                 ...
                 Message: {
                   Body: {
                     Html: {
                       ...
                       Data: `  
                       <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                          <h2 style="color: #dc3545;">NEW WHOLE FORM SUBMISSION ALERT</h2>
                    ...
                    <li>Submission ID: ${newWform._id}</li>                      
                    <li>Appointment ID: ${newAppointment._id}</li>
                      ...
                      Text: {
                        Charset: "UTF-8",
                        Data: `New Whole Form submission from ${formData.firstName} ${formData.lastName} (${email})\n\nReview in admin panel.`
                      }
                    },
                    Subject: {
                      Charset: "UTF-8",
                      Data: "🚨 New Whole Form Submission Alert"
                    }
                  },
                  Source: process.env.SES_SENDER_EMAIL
                };
           ...
           try {
             ...
             await Notification.create({
               message: `New whole form submission from ${formData.firstName} ${formData.lastName} at ${formattedSlot}`,
               formType: 'wholeForm',
               read: false
             });
           ...
           res.status(201).json({
           message: 'Whole Life Insurance Form Submission Successful', 
       ...
       const getallWforms  = asyncHandler(async (req, res) => {
         try {
           const getWforms = await Wform.find().sort({ createdAt: -1 });
       ...
       const deleteForm = asyncHandler(async (req, res) => {
         try {
           const form = await Wform.findById(req.params.id);
           if (!form) {
           ...
           await form.remove();
           res.status(200).json({ success: true, message: 'Whole Form and appointment deleted successfully' });
         } catch (error) {
...
//              formData = await Wform.findById(appointment.formId);
//            }
...
//        Your Preferred Term: ${formData.premiumTerms}` : ''}
...
//                            <li><strong>Coverage Interest:</strong> ${formData.coverage || 'Not specified'}</li>
