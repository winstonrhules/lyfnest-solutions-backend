...
// routes/scheduledEmailRoutes.js or wherever your routes are defined

const express = require('express');
const router = express.Router();
const schedulerInstance = require('../services/EmailSchedulerService');

// Get all scheduled emails
router.get('/scheduled', async (req, res) => {
  try {
    const emails = await schedulerInstance.getAllScheduledEmails();
    res.json({
      success: true,
      data: emails,
      count: emails.length
...
router.get('/scheduled/:jobId', async (req, res) => {
  try {
    const email = await schedulerInstance.getScheduledEmail(req.params.jobId);
    if (!email) {
      return res.status(404).json({
        success: false,
...
router.post('/scheduled/:jobId/send-now', async (req, res) => {
  try {
    ...
    await schedulerInstance.processJob(email);
...
router.delete('/scheduled/:jobId', async (req, res) => {
  try {
    const result = await schedulerInstance.cancelEmail(req.params.jobId);
    res.json({
      success: true,
      data: result,
      message: 'Email cancelled successfully'
...
router.get('/status', async (req, res) => {
  try {
    const status = await schedulerInstance.getStatus();
    res.json({
      success: true,
      data: status
    });
...
router.post('/schedule', async (req, res) => {
  try {
    const result = await schedulerInstance.scheduleEmail(req.body);
    res.json({
      success: true,
      data: result,
      message: 'Email scheduled successfully'
