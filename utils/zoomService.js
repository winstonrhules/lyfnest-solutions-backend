...
const processUpcomingMeetings = async (meetings, accessToken) => {
  for (const meeting of meetings) {
    try {
      ...
      // ✅ FIRST: Check if we already have this meeting in our database
      let existingZoomMeeting = await ZoomMeeting.findOne({ meetingId: meeting.id.toString() });
      ...
      // Get appointments from last 7 days that are contacted but not booked
      const availableAppointments = await Appointment.find({
        status: 'contacted',
        'zoomMeeting.meetingId': { $exists: false },
        lastContactDate: { $gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) }
      ...
      // ✅ STRATEGY 1: Exact meeting ID match (if we have stored meeting ID in scheduler URL)
      // This is the most reliable method
      matchedAppointment = availableAppointments.find(app => {
        if (app.zoomMeeting && app.zoomMeeting.meetingId === meeting.id.toString()) {
          matchScore = 100;
          return true;
        }
        return false;
      ...
      if (matchedAppointment) {
        matchReason = 'exact meeting ID match';
      }

      // ✅ STRATEGY 2: Email matching from Zoom registrants (if available)
      if (!matchedAppointment && meeting.id) {
        try {
          ...
          if (registrants.length > 0) {
            for (const registrant of registrants) {
              const emailMatch = availableAppointments.find(app => {
                const userEmail = app.user?.email || app.formData?.email || app.formId?.email;
                return userEmail && userEmail.toLowerCase() === registrant.email.toLowerCase();
              });
              
              if (emailMatch) {
                matchedAppointment = emailMatch;
                matchReason = `email match: ${registrant.email}`;
                matchScore = 90;
                break;
              }
            }
          ...
          console.log(`No registrants data for meeting ${meeting.id}`);
        }
      }

      // ✅ STRATEGY 3: Name matching from topic (existing logic but improved)
      if (!matchedAppointment && meeting.topic) {
        const topicLower = meeting.topic.toLowerCase();
        
        // Extract possible names from topic (more flexible pattern)
        const namePatterns = [
          meeting.topic.replace(/Consultation|Insurance|Financial|Meeting|Zoom/gi, '').trim(),
          meeting.topic.split('-').pop().trim(),
          meeting.topic.split('with').pop().trim()
        ];
        
        for (const namePattern of namePatterns) {
          if (namePattern.length < 3) continue;
          
          const namePatternLower = namePattern.toLowerCase();
          
          matchedAppointment = availableAppointments.find(app => {
            const userData = app.formData || app.formId || app.user || {};
            const firstName = (userData.firstName || '').toLowerCase();
            const lastName = (userData.lastName || '').toLowerCase();
            const fullName = `${firstName} ${lastName}`.trim().toLowerCase();
            
            // Multiple matching strategies
            const exactMatch = fullName === namePatternLower;
            const bothNamesMatch = firstName && lastName && 
                                 namePatternLower.includes(firstName) && 
                                 namePatternLower.includes(lastName);
            const firstNameMatch = firstName && namePatternLower.includes(firstName) && firstName.length >= 3;
            const lastNameMatch = lastName && namePatternLower.includes(lastName) && lastName.length >= 3;
            
            if (exactMatch) {
              matchScore = 85;
              return true;
            } else if (bothNamesMatch) {
              matchScore = 80;
              return true;
          ...
          if (matchedAppointment) {
            matchReason = `name pattern match: "${namePattern}"`;
            break;
          }
        }
      }

      // ✅ STRATEGY 4: Time-based matching with fuzzy time window
      if (!matchedAppointment) {
        // Allow ±2 hours time window for matching
        const timeWindow = 2 * 60 * 60 * 1000; // 2 hours in milliseconds
        
        matchedAppointment = availableAppointments.find(app => {
          const appTime = new Date(app.assignedSlot).getTime();
          const meetingTime = meetingStartTime.getTime();
          const timeDiff = Math.abs(appTime - meetingTime);
          
          return timeDiff <= timeWindow;
        });
        
        if (matchedAppointment) {
          matchReason = 'time window match (±2 hours)';
          matchScore = 60;
        }
      }

      // ✅ STRATEGY 5: Fallback - most recent contacted appointment
      if (!matchedAppointment && availableAppointments.length > 0) {
        // If only one appointment in recent days, use it as fallback
        const recentAppointments = availableAppointments.filter(app => 
          new Date(app.lastContactDate) > new Date(Date.now() - 2 * 24 * 60 * 60 * 1000)
        );
        
        if (recentAppointments.length === 1) {
          matchedAppointment = recentAppointments[0];
          matchReason = 'fallback - single recent appointment';
          matchScore = 50;
        }
      }

      // ✅ Only proceed if we have a reasonable match
      if (matchedAppointment && matchScore >= 50) {
        console.log(`✅ Matched appointment ${matchedAppointment._id} with meeting ${meeting.id} - Reason: ${matchReason} (Score: ${matchScore})`);
        
        // Create ZoomMeeting record
        const newZoomMeeting = new ZoomMeeting({
          appointment: matchedAppointment._id,
          meetingId: meeting.id.toString(), // Ensure string format
          joinUrl: meeting.join_url,
          startUrl: meeting.start_url || '',
          hostEmail: meeting.host_email,
          createdAt: new Date(meeting.created_at) || new Date(),
        ...
        // Create notification for manual review if match score is low
        if (matchScore < 70) {
          await Notification.create({
            message: `Low confidence match (${matchScore}%): ${matchedAppointment.user?.firstName || 'Unknown'} - Meeting: "${meeting.topic}"`,
            formType: 'zoom_booking',
            read: false,
            appointmentId: matchedAppointment._id,
            priority: 'medium'
          });
          console.log(`⚠️  Low confidence match created notification for manual review`);
        }
      } else {
        console.log(`❌ No suitable match found for meeting ${meeting.id}. Best match score: ${matchScore}`);
        
        // Create notification for unmatched meeting
        await Notification.create({
          message: `Unmatched Zoom meeting: "${meeting.topic}" at ${meetingStartTime.toLocaleString()}`,
          formType: 'zoom_unmatched',
          read: false,
          priority: 'high'
        });
      }
