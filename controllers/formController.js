...
//                     Data: `  
//                     <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
//                         <h2 style="color: #dc3545;">NEW FORM SUBMISSION</h2>
...
//             message: `New form submission from ${formData.firstName} ${formData.lastName} at ${formattedSlot}`,
//             formType: 'mainForm',
//             read: false
//           });
//         } catch (notifError) {
...
const sendMicrosoftEmail = async (to, subject, htmlContent, textContent) => {
  ...
  const email = {
    message: {
      ...
      // Disable reply-to by setting a no-reply address and removing reply-to headers
      replyTo: [],
      from: {
        emailAddress: {
          address: process.env.AZURE_NO_REPLY_EMAIL || "noreply@lyfnest.com",
          name: "LyfNest Solutions"
        }
      },
      // Additional headers to discourage replies
  ...
  await graphClient.api('/users/' + process.env.AZURE_SENDER_EMAIL + '/sendMail')
    .post(email);
};
...
const sendEmailVerification = asyncHandler(async (req, res) => {
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p style="color: #999; font-size: 12px;">
        This is an automated message from LyfNest Solutions. Please do not reply to this email.
      </p>
    </div>`;

  const textContent = `LyfNest Solutions will NEVER proactively call or text you for this code. DO NOT share it.
  Your verification code is: ${token}
  This code is active for 10 minutes from the time of request.
  ...
  try {
    await sendMicrosoftEmail(email, "Verify Your Email Address", htmlContent, textContent);
    res.status(200).json({ message: 'Verification email sent', expiresAt });
  } catch (error) {
        ...
        try {
          const userHtmlContent = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <p>We've received your information and will contact you between 24-72 hours</p>
              <div style="background: #f5f5f5; padding: 20px; margin: 20px 0;">
                Contact our support team at <a href="mailto:${process.env.MICROSOFT_SUPPORT_EMAIL || 'support@lyfnest.com'}">${process.env.MICROSOFT_SUPPORT_EMAIL || 'support@lyfnest.com'}</a>
              </div>
              <p style="color: #616161;">
                <strong>Please note:</strong>
                <ul>
          ...
          const userTextContent = `Thank you for submitting your form to LyfNest Solutions!\n\nWe've received your information and will contact you within 24-72 hours.\n\nSecurity notice: Never share personal information via email.\n\nThis is an automated message. Please do not reply.`;
          
          await sendMicrosoftEmail(email, "Form Submission Confirmation", userHtmlContent, userTextContent);
          userEmailSent = true;
        } catch (userMailError) {
          console.error("User Email Confirmation Failed", userMailError);
        }
     ...
     let adminAlertSent = false;
        try {
          ...
          if (admins.length > 0) {
            ...
            const adminTextContent = `New submission from ${formData.firstName} ${formData.lastName} (${email})\n\nReview in admin panel.`;
            
            // Send to each admin individually or BCC
            for (const adminEmail of adminEmails) {
              await sendMicrosoftEmail(adminEmail, "🚨 New Form Submission Alert", adminHtmlContent, adminTextContent);
            }
            adminAlertSent = true;
          }
    } catch (adminEmailError) {
