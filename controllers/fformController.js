...
const getallFforms = asyncHandler(async(req, res)=>{
  try{
    const getFforms = await Fform.find()
    res.status(200).json(getFforms)
  }
  catch(error){
...
module.exports = {initialVerificationChecks, sendEmailVerification, verifyCode, verifyEmailCode, submissionForm, getallFforms}